class Admin::PostsController < Admin::ApplicationController
  ...
  def create
    ...
    if @post.save
      ...
      flash[:alert] = 'There was problem while creating your post'
      render :new
    end
  ...
  def edit
    @post = Post.find(params[:id]) #after editing, on clicking the (update) button, it goes to update method
  end

  def update
    @post = Post.find(params[:id]) #finding a post (based on id) to be updated
    if @post.update(post_params)
      redirect_to admin_posts_url, notice: 'Post was successfully updated'
    else
      flash[:alert] = 'There was a problem while updating your post'
      render :edit
    end
  end
